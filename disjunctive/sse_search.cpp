...
int main()
{
    ...
    unsigned int n_iterations = 1000;//not used here
    ...
    std::string queryvector_file = "./test_vectors/HW2_query/HW10_hex_queryvector.csv";
    std::string testvector_file = "./test_vectors/HW2_query/HW10_testvector.csv";
    std::string freqvector_file = "./test_vectors/HW2_query/HW10_freqvector.csv";
    std::string binvector_file = "./test_vectors/HW2_query/HW10_binvector.csv";
    ...
    for(unsigned int i=0;i<n_testvectors;++i){
        ...
        ss.clear();
		ss << raw_qv_data.at(i);//Get the current bin index row
		row_qv_data.clear();
		while(std::getline(ss,s,',') && !ss.eof()) {
		    if(!s.empty()){
        ...
        cout << "Result size: " << result_union.size() << endl;
